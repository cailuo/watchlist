...
class WatchlistTestCase(unittest.TestCase):
    ...
    # 辅助方法，用户登录用户
    def login(self):
        # 字典中的键为表单<input>元素的name属性值，作为登录表单的输入数据
    ...
    def test_update_item(self):
        ...
        # 测试更新条目操作
        response = self.client.post('/movie/edit/1', data=dict(title='Test Movie Edited', year='2019'), follow_redirects=True)
        data = response.get_data(as_text=True)
    ...
    def test_settings(self):
        ...
        # 测试更新设置
        response = self.client.post('/settings', data=dict(name='cailuo',), follow_redirects=True)
        data = response.get_data(as_text=True)
        self.assertIn('Settings updated.', data)
        self.assertIn('cailuo', data)
    ...
    def test_forge_command(self):
        # invoke方法返回命令执行结果对象，它的output属性返回命令的输出信息
        # 传入命令函数对象
    ...
    def test_admin_command(self):
        ...
        db.create_all()
        # 使用args关键字直接给出命令参数列表
        result = self.runner.invoke(args=['admin', '--username', 'cailuo', '--password', '123'])
        self.assertIn('Creating user...', result.output)
        self.assertIn('Done', result.output)
        self.assertEqual(User.query.count(), 1)
        self.assertEqual(User.query.first().username, 'cailuo')
